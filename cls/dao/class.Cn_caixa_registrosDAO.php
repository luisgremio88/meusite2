<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cn_caixa_registros" 
 */
class Cn_caixa_registrosDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cn_caixa_registros
	 */
	public function insert($obj) {
		...
		$data_time = $this->dba->quote($obj->getData_time());
		$id_fechamento = $this->dba->quote($obj->getId_fechamento());
		
		//montar o comando SQL
		$sql = "insert into cn_caixa_registros 
				(
				nome,
				descricao,
				id_conta,
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cn_caixa_registros
	...
	public function update($obj) {
		...
		$valor = $obj->getValor();
		$data_time = $obj->getData_time();
		$id_fechamento = $obj->getId_fechamento();
        ...
        if (!empty($data_time) || is_numeric($data_time)) {
        	$campos .= "data_time=".$this->dba->quote($data_time).", ";
        }
        if (!empty($id_fechamento) || is_numeric($id_fechamento)) {
        	$campos .= "id_fechamento=".$this->dba->quote($id_fechamento).", ";
        }
        $campos = substr($campos,0,strrpos($campos,','));
        
		//montar o comando SQL
        $sql = "update cn_caixa_registros set
				$campos
				where id = $id";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cn_caixa_registros
	 */
	public function delete($obj) {
		...
		//montar o comando SQL
		$sql = "delete from cn_caixa_registros 
				where id = $id";
	...
	/**
	 * metodo que retorna os regsitros da tabela Cn_caixa_registros conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cn_caixa_registros $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cn_caixa_registros
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cn_caixa_registros WHERE Field = '".$coluna."'");
