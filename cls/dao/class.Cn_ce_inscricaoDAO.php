<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cn_ce_inscricao" 
 */
class Cn_ce_inscricaoDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cn_ce_inscricao
	 */
	public function insert($obj) {
		...
		$adicionais = $this->dba->quote($obj->getAdicionais());
		$status = $this->dba->quote($obj->getStatus());
		$total = $this->dba->quote($obj->getTotal());
		
		//montar o comando SQL
		$sql = "insert into cn_ce_inscricao 
				(
				id_curso_evento,
				data,
				cpf,
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cn_ce_inscricao
	...
	public function update($obj) {
        ...
        if (!empty($adicionais) || is_numeric($adicionais)) {
        	$campos .= "adicionais=".$this->dba->quote($adicionais).", ";
        }
        if (!empty($status) || is_numeric($status)) {
        	$campos .= "status=".$this->dba->quote($status).", ";
        }
        if (!empty($total) || is_numeric($total)) {
        	$campos .= "total=".$this->dba->quote($total).", ";
        }
        $campos = substr($campos,0,strrpos($campos,','));
        
		//montar o comando SQL
        $sql = "update cn_ce_inscricao set
				$campos
				where id = $id";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cn_ce_inscricao
	 */
	public function delete($obj) {
		...
		//montar o comando SQL
		$sql = "delete from cn_ce_inscricao 
				where id = $id";
	...
	/**
	 * metodo que retorna os regsitros da tabela Cn_ce_inscricao conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cn_ce_inscricao $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cn_ce_inscricao
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cn_ce_inscricao WHERE Field = '".$coluna."'");
