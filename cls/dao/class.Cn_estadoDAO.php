<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cn_estado" 
 */
class Cn_estadoDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cn_estado
	 */
	public function insert($obj) {
		//pegar os dados do objeto
		$EstadoId = $this->dba->quote($obj->getEstadoid());
		$Nome = $this->dba->quote($obj->getNome());
		$Sigla = $this->dba->quote($obj->getSigla());
		
		//montar o comando SQL
		$sql = "insert into cn_estado 
				(
				EstadoId,
				Nome,
				Sigla
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cn_estado
	...
	public function update($obj) {
        ...
        if (!empty($Sigla) || is_numeric($Sigla)) {
        	$campos .= "Sigla=".$this->dba->quote($Sigla).", ";
        }
        $campos = substr($campos,0,strrpos($campos,','));
        
		//montar o comando SQL
        $sql = "update cn_estado set
				$campos
				where  = $";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cn_estado
	 */
	...
	/**
	 * metodo que retorna os regsitros da tabela Cn_estado conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cn_estado $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cn_estado
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cn_estado WHERE Field = '".$coluna."'");
