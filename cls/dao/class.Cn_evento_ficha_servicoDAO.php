<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cn_evento_ficha_servico" 
 */
class Cn_evento_ficha_servicoDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cn_evento_ficha_servico
	 */
	public function insert($obj) {
		...
		$servico_id = $this->dba->quote($obj->getServico_id());
		$ficha_participante_id = $this->dba->quote($obj->getFicha_participante_id());
		$valor = $this->dba->quote($obj->getValor());
		
		//montar o comando SQL
		$sql = "insert into cn_evento_ficha_servico 
				(
				ficha_id,
				participante_id,
				servico_id,
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cn_evento_ficha_servico
	...
	public function update($obj) {
		...
		$servico_id = $obj->getServico_id();
		$ficha_participante_id = $obj->getFicha_participante_id();
		$valor = $obj->getValor();
        ...
        if (!empty($servico_id) || is_numeric($servico_id)) {
        	$campos .= "servico_id=".$this->dba->quote($servico_id).", ";
        }
        if (!empty($ficha_participante_id) || is_numeric($ficha_participante_id)) {
        	$campos .= "ficha_participante_id=".$this->dba->quote($ficha_participante_id).", ";
        }
		...
		//montar o comando SQL
        $sql = "update cn_evento_ficha_servico set
				$campos
				where id = $id";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cn_evento_ficha_servico
	 */
	public function delete($obj) {
		...
		//montar o comando SQL
		$sql = "delete from cn_evento_ficha_servico 
				where id = $id";
	...
	/**
	 * metodo que retorna os regsitros da tabela Cn_evento_ficha_servico conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cn_evento_ficha_servico $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cn_evento_ficha_servico
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cn_evento_ficha_servico WHERE Field = '".$coluna."'");
