<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cn_log_de_atividade" 
 */
class Cn_log_de_atividadeDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cn_log_de_atividade
	 */
	public function insert($obj) {
		//pegar os dados do objeto
		$id_usuario = $this->dba->quote($obj->getId_usuario());
		$nome_usuario = $this->dba->quote($obj->getNome_usuario());
		$ip = $this->dba->quote($obj->getIp());
		$data_hora = $this->dba->quote($obj->getData_hora());
		$descricao = $this->dba->quote($obj->getDescricao());
		
		//montar o comando SQL
		$sql = "insert into cn_log_de_atividade 
				(
				id_usuario,
				nome_usuario,
				ip,
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cn_log_de_atividade
	...
	public function update($obj) {
        ...
        if (!empty($ip) || is_numeric($ip)) {
        	$campos .= "ip=".$this->dba->quote($ip).", ";
        }
        if (!empty($data_hora) || is_numeric($data_hora)) {
        	$campos .= "data_hora=".$this->dba->quote($data_hora).", ";
        }
		...
		//montar o comando SQL
        $sql = "update cn_log_de_atividade set
				$campos
				where id = $id";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cn_log_de_atividade
	 */
	public function delete($obj) {
		...
		//montar o comando SQL
		$sql = "delete from cn_log_de_atividade 
				where id = $id";
	...
	/**
	 * metodo que retorna os regsitros da tabela Cn_log_de_atividade conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cn_log_de_atividade $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cn_log_de_atividade
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cn_log_de_atividade WHERE Field = '".$coluna."'");
