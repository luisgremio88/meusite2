<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cn_mandato" 
 */
class Cn_mandatoDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cn_mandato
	 */
	public function insert($obj) {
		//pegar os dados do objeto
		$DataInicio = $this->dba->quote($obj->getDatainicio());
		$DataFim = $this->dba->quote($obj->getDatafim());
		
		//montar o comando SQL
		$sql = "insert into cn_mandato 
				(
				DataInicio,
				DataFim
				) 
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cn_mandato
	...
	public function update($obj) {
        ...
        $campos = '';
        if (!empty($DataInicio) || is_numeric($DataInicio)) {
        	$campos .= "DataInicio=".$this->dba->quote($DataInicio).", ";
        }
        if (!empty($DataFim) || is_numeric($DataFim)) {
        	$campos .= "DataFim=".$this->dba->quote($DataFim).", ";
        }
        $campos = substr($campos,0,strrpos($campos,','));
        
		//montar o comando SQL
        $sql = "update cn_mandato set
				$campos
				where MandatoId = $MandatoId";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cn_mandato
	 */
	...
	/**
	 * metodo que retorna os regsitros da tabela Cn_mandato conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cn_mandato $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cn_mandato
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cn_mandato WHERE Field = '".$coluna."'");
