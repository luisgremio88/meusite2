<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cn_pagamento_agendado_gerado" 
 */
class Cn_pagamento_agendado_geradoDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cn_pagamento_agendado_gerado
	 */
	public function insert($obj) {
		//pegar os dados do objeto
		$id_agenda = $this->dba->quote($obj->getId_agenda());
		$mes = $this->dba->quote($obj->getMes());
		$ano = $this->dba->quote($obj->getAno());
		
		//montar o comando SQL
		$sql = "insert into cn_pagamento_agendado_gerado 
				(
				id_agenda,
				mes,
				ano
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cn_pagamento_agendado_gerado
	 * para remover o valor de uma coluna, atribua a string '!NULL!'
	 */
	public function update($obj) {
		//pegar os dados do objeto
		$id = $obj->getId();
		$id_agenda = $obj->getId_agenda();
		$mes = $obj->getMes();
		$ano = $obj->getAno();
		
        $campos = '';
        if (!empty($id_agenda) || is_numeric($id_agenda)) {
        	$campos .= "id_agenda=".$this->dba->quote($id_agenda).", ";
        }
		...
		//montar o comando SQL
        $sql = "update cn_pagamento_agendado_gerado set
				$campos
				where id = $id";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cn_pagamento_agendado_gerado
	 */
	public function delete($obj) {
		...
		//montar o comando SQL
		$sql = "delete from cn_pagamento_agendado_gerado 
				where id = $id";
	...
	/**
	 * metodo que retorna os regsitros da tabela Cn_pagamento_agendado_gerado conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cn_pagamento_agendado_gerado $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cn_pagamento_agendado_gerado
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cn_pagamento_agendado_gerado WHERE Field = '".$coluna."'");
