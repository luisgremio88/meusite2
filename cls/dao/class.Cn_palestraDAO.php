<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cn_palestra" 
 */
class Cn_palestraDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cn_palestra
	 */
	public function insert($obj) {
		//pegar os dados do objeto
		$Evento = $this->dba->quote($obj->getEvento());
		...
		$DataPalestra = $this->dba->quote($obj->getDatapalestra());
		
		//montar o comando SQL
		$sql = "insert into cn_palestra 
				(
				Evento,
				Descricao,
				DataCriacao,
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cn_palestra
	 * para remover o valor de uma coluna, atribua a string '!NULL!'
	 */
	public function update($obj) {
		//pegar os dados do objeto
		$PalestraId = $obj->getPalestraid();
		$Evento = $obj->getEvento();
		$Descricao = $obj->getDescricao();
		$DataCriacao = $obj->getDatacriacao();
		$UsuarioId = $obj->getUsuarioid();
		$DataPalestra = $obj->getDatapalestra();
		
        $campos = '';
        if (!empty($Evento) || is_numeric($Evento)) {
        	$campos .= "Evento=".$this->dba->quote($Evento).", ";
        }
        ...
        if (!empty($DataPalestra) || is_numeric($DataPalestra)) {
        	$campos .= "DataPalestra=".$this->dba->quote($DataPalestra).", ";
        }
        $campos = substr($campos,0,strrpos($campos,','));
        
		//montar o comando SQL
        $sql = "update cn_palestra set
				$campos
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cn_palestra
	 */
	...
	/**
	 * metodo que retorna os regsitros da tabela Cn_palestra conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cn_palestra $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cn_palestra
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cn_palestra WHERE Field = '".$coluna."'");
