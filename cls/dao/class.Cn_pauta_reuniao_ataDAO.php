<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cn_pauta_reuniao_ata" 
 */
class Cn_pauta_reuniao_ataDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cn_pauta_reuniao_ata
	 */
	public function insert($obj) {
		...
		$DataAta = $this->dba->quote($obj->getDataata());
		$DataCriacao = $this->dba->quote($obj->getDatacriacao());
		$PautaId = $this->dba->quote($obj->getPautaid());
		
		//montar o comando SQL
		$sql = "insert into cn_pauta_reuniao_ata 
				(
				Horario,
				Titulo,
				Texto,
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cn_pauta_reuniao_ata
	 * para remover o valor de uma coluna, atribua a string '!NULL!'
	 */
	public function update($obj) {
		//pegar os dados do objeto
		$PautaReuniaoAtaId = $obj->getPautareuniaoataid();
		$Horario = $obj->getHorario();
		$Titulo = $obj->getTitulo();
		$Texto = $obj->getTexto();
		$DataAta = $obj->getDataata();
		$DataCriacao = $obj->getDatacriacao();
		$PautaId = $obj->getPautaid();
        ...
        if (!empty($DataAta) || is_numeric($DataAta)) {
        	$campos .= "DataAta=".$this->dba->quote($DataAta).", ";
        }
        if (!empty($DataCriacao) || is_numeric($DataCriacao)) {
        	$campos .= "DataCriacao=".$this->dba->quote($DataCriacao).", ";
        }
        if (!empty($PautaId) || is_numeric($PautaId)) {
        	$campos .= "PautaId=".$this->dba->quote($PautaId).", ";
        }
        $campos = substr($campos,0,strrpos($campos,','));
        
		//montar o comando SQL
        $sql = "update cn_pauta_reuniao_ata set
				$campos
				where PautaReuniaoAtaId = $PautaReuniaoAtaId";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cn_pauta_reuniao_ata
	 */
	public function delete($obj) {
		...
		//montar o comando SQL
		$sql = "delete from cn_pauta_reuniao_ata 
				where PautaReuniaoAtaId = $PautaReuniaoAtaId";
	...
	/**
	 * metodo que retorna os regsitros da tabela Cn_pauta_reuniao_ata conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cn_pauta_reuniao_ata $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cn_pauta_reuniao_ata
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cn_pauta_reuniao_ata WHERE Field = '".$coluna."'");
