<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cn_usuarios_associados_token" 
 */
class Cn_usuarios_associados_tokenDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cn_usuarios_associados_token
	 */
	public function insert($obj) {
		//pegar os dados do objeto
		$id_usuarios = $this->dba->quote($obj->getId_usuarios());
		$token = $this->dba->quote($obj->getToken());
		$data_hora_registro = $this->dba->quote($obj->getData_hora_registro());
		
		//montar o comando SQL
		$sql = "insert into cn_usuarios_associados_token 
				(
				id_usuarios,
				token,
				data_hora_registro
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cn_usuarios_associados_token
	 * para remover o valor de uma coluna, atribua a string '!NULL!'
	 */
	public function update($obj) {
		//pegar os dados do objeto
		$id = $obj->getId();
		$id_usuarios = $obj->getId_usuarios();
		$token = $obj->getToken();
		$data_hora_registro = $obj->getData_hora_registro();
		
        $campos = '';
        if (!empty($id_usuarios) || is_numeric($id_usuarios)) {
        	$campos .= "id_usuarios=".$this->dba->quote($id_usuarios).", ";
        }
		...
		//montar o comando SQL
        $sql = "update cn_usuarios_associados_token set
				$campos
				where id = $id";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cn_usuarios_associados_token
	 */
	public function delete($obj) {
		...
		//montar o comando SQL
		$sql = "delete from cn_usuarios_associados_token 
				where id = $id";
	...
	/**
	 * metodo que retorna os regsitros da tabela Cn_usuarios_associados_token conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cn_usuarios_associados_token $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cn_usuarios_associados_token
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cn_usuarios_associados_token WHERE Field = '".$coluna."'");
