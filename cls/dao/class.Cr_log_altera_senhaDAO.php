<?php
		/**
 * Classe que representa os comandos de acesso aos dados da tabela "Cr_log_altera_senha" 
 */
class Cr_log_altera_senhaDAO extends PDO
{
	/**
	...
	/**
	 * metodo que faz a insercao de um registro na tabela Cr_log_altera_senha
	 */
	public function insert($obj) {
		//pegar os dados do objeto
		$ip = $this->dba->quote($obj->getIp());
		$data_hora_registro = $this->dba->quote($obj->getData_hora_registro());
		$usuario = $this->dba->quote($obj->getUsuario());
		
		//montar o comando SQL
		$sql = "insert into cr_log_altera_senha 
				(
				ip,
				data_hora_registro,
				usuario
	...
	/**
	 * metodo que faz a atualizacao de um registro na tabela Cr_log_altera_senha
	...
	public function update($obj) {
        ...
        if (!empty($usuario) || is_numeric($usuario)) {
        	$campos .= "usuario=".$this->dba->quote($usuario).", ";
        }
        $campos = substr($campos,0,strrpos($campos,','));
        
		//montar o comando SQL
        $sql = "update cr_log_altera_senha set
				$campos
				where id = $id";
	...
	/**
	 * metodo que faz a exclusao de um registro na tabela Cr_log_altera_senha
	 */
	public function delete($obj) {
		...
		//montar o comando SQL
		$sql = "delete from cr_log_altera_senha 
				where id = $id";
	...
	/**
	 * metodo que retorna os regsitros da tabela Cr_log_altera_senha conforme o filtro informado
	...
	public function select($where='', $order='') {
		...
		$sql = "select * from cr_log_altera_senha $where $order";
    ...
    /**
	 * metodo que retorna o ultimo id cadastrado na tabela Cr_log_altera_senha
	 */
    ...
    public function metaValues($coluna, $primeiro = NULL, $excluir = NULL) {
		...
		$res = $this->dba->query("SHOW COLUMNS FROM cr_log_altera_senha WHERE Field = '".$coluna."'");
