<?php
require_once('../inc/inc.autoload.php');
require_once('../inc/inc.globals.php');
require_once('./config/inc.functions.php');
checkSession();

//página de conteúdo (content)
$npag = $_SERVER['PHP_SELF']; 
$npag = substr($npag, strrpos($npag,'/')+1);
$page = substr($npag, 0, strrpos($npag,'.')).'.html';

//layout "mestre"
$basedir = './static/';
$tpl = new TemplatePower($basedir.'_master.html'); 
//$tpl->assignInclude('header', $basedir.'_header2.html');
$tpl->assignInclude('content', $basedir.$page);
$tpl->prepare();

// ---------- INI PROCESSAMENTO ------------

// grupo dessa página
$grppag = '3-conteudos';
//controle do acesso ao menu
include_once('_menu.php');

try 
{
    $idpenv = request('id');
    if (!is_numeric($idpenv)) {
        // busca a última pauta cadastrada, para exibir
        $host = "https://www.colnotrs.app.br/ws/v1/pauta-reuniao.php";
        $body = '{"id_tabelionato":'.$idt.' , "limit":1 , "offset":0}';
        $head = array('Content-Type:application/json' , 'Content-Length:'.strlen($body) , 'Accept: text/json');
        $api = new ApiRest($host);
        $res = $api->post($body, $head); 
        $objeto = json_decode($res);
        if ($objeto->ok == true) {
            foreach ($objeto->msg as $vetobj) {
                $vet = get_object_vars($vetobj);
                $idpenv = $vet['id'];
            }
        } else {
            // $tpl->newBlock('bug');
            // $tpl->assign('msg', 'Falha na comunicação com a API');
            header('location: board');
            exit;
        }
    }

    //pauta escolhida (id)
    $host = "https://www.colnotrs.app.br/ws/v1/pauta-reuniao-details.php";
    $body = '{"id_tabelionato":'.$idt.' , "id_pauta":'.$idpenv.'}';
    $head = array('Content-Type:application/json' , 'Content-Length:'.strlen($body) , 'Accept: text/json');
    $api = new ApiRest($host);
    $res = $api->post($body, $head); 
    $objeto = json_decode($res);
    if ($objeto->ok == true) {
        //pauta em detalhes
        foreach ($objeto->msg as $vetobj) {
            $vet = get_object_vars($vetobj);
            $idp = $vet['id'];
            $tit = $vet['Titulo'];
            $txt = $vet['Texto'];
            $dat = $vet['DataPauta'];
            $hor = $vet['Horario'];
            $vid = $vet['video'];
            $anx = $vet['anexo'];
            //ajustes
            $dat = datetime_date_ptbr($dat);
            //exibe na tela
            if ($idp == $idpenv) {
                $tpl->gotoBlock('_ROOT');
                $tpl->assign('idp', $idp);
                $tpl->assign('tit', $tit);
                $tpl->assign('txt', $txt);
                $tpl->assign('dat', $dat);
                $tpl->assign('hor', $hor);
                if (!empty($vid)) {
                    $tpl->newBlock('video');
                    $tpl->assign('vid', $vid);
                } else {
                    $tpl->newBlock('no-video');
                }
                if (!empty($anx)) {
                    $tpl->newBlock('anexo');
                    $tpl->assign('anx', $anx);
                } else {
                    $tpl->newBlock('no-anexo');
                }
                break;
            }
        }
    } else {
        $tpl->newBlock('bug');
        $tpl->assign('msg', 'Falha na comunicação com a API');
    }

    //lista das pautas anteriores
    $host = "https://www.colnotrs.app.br/ws/v1/pauta-reuniao.php";
    $body = '{"id_tabelionato":'.$idt.' , "limit":10 , "offset":0}';
    $head = array('Content-Type:application/json' , 'Content-Length:'.strlen($body) , 'Accept: text/json');
    $api = new ApiRest($host);
    $res = $api->post($body, $head); 
    $objeto = json_decode($res);
    if ($objeto->ok == true) {
        foreach ($objeto->msg as $vetobj) {
            $vet = get_object_vars($vetobj);
            $idp = $vet['id'];
            $tit = $vet['Titulo'];
            $dat = $vet['DataPauta'];
            $dat = datetime_date_ptbr($dat);
            $tpl->newBlock('pautas-ant');
            $tpl->assign('idp', $idp);
            $tpl->assign('tit', $tit);
            $tpl->assign('dat', $dat);
        }
    } else {
        $tpl->newBlock('bug');
        $tpl->assign('msg', 'Falha na comunicação com a API');
    }

}
catch(Exception $ex) {
    $tpl->newBlock('bug');
    $tpl->assign('msg', $ex->getMessage());
}

// ---------- FIM PROCESSAMENTO ------------

$tpl->printToScreen();
?>