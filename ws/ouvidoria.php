...
$mail->addAddress('mateus@dedstudio.com.br');
...
$mail->Subject = '[Colégio Notarial RS] Ouvidoria | ' . $assunto;
...
if (!$mail->send()) {
    $array = array("error" => "true", "type" => "mailer", "msg" => "Não foi possível enviar seu email. Entre em contato com o suporte!");
    header('Content-type: application/json');
    echo json_encode($array);
    exit;
} else {
    $sql   = "INSERT INTO cr_ouvidoria (titulo, nome, email, telefone, texto, status, data) VALUES ('$assunto', '$name', '$email', '$telefone', '$message', 1, NOW())";
    $query = $dba->query($sql);
}
