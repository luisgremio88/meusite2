...
$sql3   = "SELECT id FROM cn_revistas WHERE status = 1 ORDER BY data DESC";
$query3 = $dba->query($sql3);
$total  = $dba->rows($query3);

$sql2   = "SELECT * FROM cn_revistas WHERE status = 1 ORDER BY data DESC LIMIT $inicio, $registros"; // print_r($sql2);
...
if ($qntd2 > 0) {	
	for ($j=0; $j<$qntd2; $j++) {
		...
		$imagem     = '';
		if (file_exists('../img/revistas/'.$id.'.jpg')) {
			$imagem = 'https://colnotrs.app.br/img/revistas/'.$id.'.jpg?v='.date('YmdHmi');
		}
...
$array = array("success" => "true", "revistas" => $array_revistas, "total" => $total, "pagina" => $pagina);
