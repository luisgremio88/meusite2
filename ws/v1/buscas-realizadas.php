...
if (isset($obj['id_tabelionato']) && !empty($obj['id_tabelionato'])) 
{
    ...
    //lista as buscas já gravadas no banco
    $sqlbus = "select bt.data, bt.nome as nome_testador, bt.cpf as cpf_testador, c.nome as nome_cliente, c.cpf as cpf_cliente
                from cn_busca_testamentos bt
                inner join cn_clientes c on c.ClienteId = bt.id_cliente
                where 
                bt.id_tabelionato = '$id_tabelionato' and 
                bt.nome like '%$nome_pesquisa%' and 
                bt.cpf like '%$cpf_pesquisa%' ";
    $resbus = $dba->query($sqlbus);
    $qtdbus = $dba->rows($resbus);
    if ($qtdbus > 0) {
        $vettmp = array();
        for ($i=0; $i<$qtdbus; $i++) {
            $vetbus = $dba->fetch($resbus);
            array_push($vettmp, $vetbus);
        }
