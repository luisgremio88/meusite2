...
if(isset($obj["id_tabelionato"], $obj["limit"], $obj["offset"])) {
    ...
    $sql    = "SELECT * FROM cn_testamentos 
    WHERE TabelionatoId = ".$obj["id_tabelionato"]." AND
        Nome like '%$nome_pesquisa%' and 
        Cpf like '%$cpf_pesquisa%'
    ORDER BY id DESC LIMIT ".$obj["limit"]." OFFSET ".$obj["offset"].";";
    $query  = $dba->query($sql);
    $qntd   = $dba->rows($query);
    $result = [];
    if ($qntd > 0) {
